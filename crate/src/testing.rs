//! A throwaway directory tree, for the two modules that genuinely need
//! a filesystem.
//!
//! `detect/` never does — that is the point of the split — so this is
//! only ever used by `walk`, `scan` and the MCP path tool.
//!
...
//! spellings of the same directory — and every path comparison between
...
impl TempTree {
    pub(crate) fn new(name: &str) -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("unicode-le-{name}-{}-{unique}", std::process::id()));
